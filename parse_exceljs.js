  ...
  workbook.eachSheet((sheet, sheetId) => {
    ...
    for (let r = 1; r <= sheet.rowCount; r++) {
      ...
      for (let c = 1; c <= sheet.columnCount; c++) {
        ...
        if (cell.value && typeof cell.value === 'string' && cell.value.trim() === 'AA') {
          let grid = [];
          for (let i = 0; i < 13; i++) {
            let gridRow = [];
            ...
            for (let j = 0; j < 13; j++) {
               ...
               const val = cLocal.value ? String(cLocal.value).trim() : null;
               
               // Extract cell background color
               let bg = null;
               if (cLocal.fill && cLocal.fill.fgColor && cLocal.fill.fgColor.argb) {
                 bg = cLocal.fill.fgColor.argb;
               } else if (cLocal.fill && cLocal.fill.bgColor && cLocal.fill.bgColor.argb) {
